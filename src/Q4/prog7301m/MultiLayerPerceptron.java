...
public class MultiLayerPerceptron implements Serializable {
    ...
    public MultiLayerPerceptron(int[] layerSizes, Activations.Activation activation) {
        ...
        for (int i = 0; i < layerSizes.length; i++)
            layers[i] = (i==0) ? new Layer(layerSizes[i], 0) : new Layer(layerSizes[i], layerSizes[i-1]);
    }
        ...
        for (int k = 1; k < layers.length; k++) {
            for (int i = 0; i < layers[k].Size; i++) {
                ...
                for (int j = 0; j < layers[k-1].Size; j++) {
                    result+= layers[k].Neurons[i].Weights[j] * layers[k-1].Neurons[i].Value;
                }
    ...
    public double backPropagate(double[] x, double[] output, double learningRate) {
        ...
        for (int i = 0; i < layers[layers.length-1].Size; i++) {
            ...
            layers[layers.length-1].Neurons[i].Delta = error * activation.activateDeriv(result[i]);
        }
        ...
        for (int k = layers.length-2; k >= 0; k--) {
            for (int i = 0; i < layers[k].Size; i++) {
                ...
                layers[k].Neurons[i].Delta = error * activation.activateDeriv(layers[k].Neurons[i].Value);
            }
    ...
    public double accuracy(double[][] X, double[][] output) {
        ...
        for (int i = 0; i < X.length; i++) {
            ...
            for (int j = 0; j < output[i].length; j++) {error += Math.abs(result[j] - output[i][j]);}
        }
        return 1 - error / (X.length *output[0].length);
    }

    public void train(double[][] X_train, double[][] y_train, int epochs, double learningRate) {
